...
class ResponseRepository {
    ...
    insertProcessingRun({ transcriptionId, parserVersion, scoringMode, expectedResponseDigits, subtractionValue, startingNumber }) {
    ...
    getProcessingRunById(id) {
        return this._db.prepare('SELECT * FROM processing_runs WHERE id = ?').get(id) || null;
    }

    getLatestProcessingRunForTranscription(transcriptionId) {
        return this._db.prepare(
            'SELECT * FROM processing_runs WHERE transcription_id = ? ORDER BY created_at DESC LIMIT 1'
        ).get(transcriptionId) || null;
    }

    listProcessingRunsForTranscription(transcriptionId) {
        return this._db.prepare(
    ...
    insertResponses(processingRunId, responses) {
    ...
    listResponsesForRun(processingRunId) {
        return this._db.prepare(
